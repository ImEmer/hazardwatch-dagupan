...
const BarangaySidebar = () => {
  ...
  const sidebar = <aside className={`fixed inset-y-0 left-0 z-50 flex h-screen w-64 flex-col border-r transition-transform duration-300 lg:z-40 lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'} ${isDark ? 'border-[#2e303a] bg-[#0a0b0f]' : 'border-slate-200 bg-white'}`}>
    <div className={`flex items-center justify-between border-b px-5 py-4 ${isDark ? 'border-[#2e303a]' : 'border-slate-200'}`}><NavLink to={`${barangayPath}/dashboard`} onClick={() => setIsOpen(false)} className="text-lg font-bold tracking-tight text-[#3b82f6]">HazardWatch</NavLink><div className="flex items-center gap-2"><NotificationBell /><button type="button" onClick={() => setIsOpen(false)} className="rounded-lg p-2 text-gray-400 hover:bg-white/10 lg:hidden" aria-label="Close menu"><MenuIcon close /></button><button type="button" onClick={toggleTheme} className={`relative hidden h-8 w-14 items-center rounded-full border p-1 transition lg:flex ${isDark ? 'border-[#2e303a] bg-[#14151d]' : 'border-slate-200 bg-slate-100'}`} aria-label={`Switch to ${isDark ? 'light' : 'dark'} theme`} title={`Switch to ${isDark ? 'light' : 'dark'} theme`}><span className={`flex h-6 w-6 items-center justify-center rounded-full transition-transform ${isDark ? 'translate-x-0 bg-slate-800 text-blue-200' : 'translate-x-6 bg-white text-amber-500 shadow-sm'}`}><span aria-hidden="true">{isDark ? '◐' : '☼'}</span></span></button></div></div>
