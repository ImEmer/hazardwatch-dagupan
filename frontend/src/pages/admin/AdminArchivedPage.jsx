...
const AdminArchivedPage = ({ basePath = '/admin' }) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    api.get('/reports/archived', { params: { page, limit: 10 }, headers: { Authorization: `Bearer ${token}` } })
      .then((response) => { if (!cancelled) { setReports(response.data?.reports || []); setPagination(response.data?.pagination || { page, limit: 10, total: 0, pages: 1 }); } })
