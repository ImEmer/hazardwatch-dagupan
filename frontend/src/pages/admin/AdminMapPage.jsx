...
const AdminMapPage = () => {
  ...
  const { reports, loading, error, onBoundsChange, retry } = useViewportReports({ endpoint: '/reports', token, includeResolved: mapPreferences.showResolved, enabled: Boolean(token) });
  ...
  }, [mapPreferences.showResolved, reports, statusFilter]);
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {['all', 'Pending', 'In Progress'].map((status) => (
              <button
                key={status}
                onClick={() => { setStatusFilter(status); setSelectedReport(null); }}
      ...
      <div className="grid min-h-0 flex-1 gap-6 overflow-hidden xl:grid-cols-[2fr_1fr]">
        <div className={`relative min-h-0 overflow-hidden rounded-2xl border p-2 shadow-xl ${isDark ? 'border-[#2e303a] bg-[#14151d]' : 'border-slate-200 bg-white'}`}>
          <div className="relative h-full min-h-0 overflow-hidden rounded-xl">
            ...
            <InteractiveMap reports={filteredReports} height="100%" colorBy="status" showHeatmap={showHeatmap} flyTo={flyTo} onBoundsChange={onBoundsChange} mapPreferences={mapPreferences} onReportSelect={setSelectedReport} />
        ...
        <MapReportList endpoint="/reports" token={token} includeResolved={mapPreferences.showResolved} status={statusFilter} selectedReport={selectedReport} onSelectReport={setSelectedReport} />
      </div>
    </div>
  );
};
