...
const tabs = [
  ...
  { key: 'admin', label: 'Admin', roles: ['admin', 'superadmin'] },
...
const SuperAdminActivityPage = () => {
  ...
  return <main className="space-y-6">
    ...
    <div className={`rounded-2xl border shadow-xl ${isDark ? 'border-[#2e303a] bg-[#14151d]' : 'border-slate-200 bg-white'}`}>{loading ? <p className={`p-8 text-center ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>Loading activity...</p> : !visibleActivities.length ? <p className={`p-8 text-center ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>No activity recorded yet.</p> : <div className={`divide-y ${isDark ? 'divide-[#2e303a]' : 'divide-slate-200'}`}>{visibleActivities.map((activity) => <div key={activity._id} className="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className={isDark ? 'text-white' : 'text-slate-900'}>{activity.message || activity.action}</p><span className="mt-1 inline-flex rounded-full bg-blue-500/10 px-2 py-1 text-xs capitalize text-blue-400">{activity.actorRole || activity.role}</span></div><time dateTime={activity.createdAt} className={`text-xs ${isDark ? 'text-gray-500' : 'text-slate-400'}`}>{new Date(activity.createdAt).toLocaleString()}</time></div>)}</div>}</div>
  </main>;
};
