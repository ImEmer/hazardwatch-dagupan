...
const report = {
  id: 'HR-2041',
  title: 'Flooded road near Bonuan market',
  status: 'Monitoring',
  priority: 'High',
  barangay: 'Bonuan',
  summary: 'Water has pooled across the main access road after heavy rainfall, affecting local traffic and pedestrian movement.',
  details: [
    'Road surface partially submerged near the market lane.',
    'Traffic cones placed to redirect vehicles away from the flooded section.',
    'Barangay responders are monitoring drainage flow and debris.'
  ],
};

const BonuanReportDetail = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <p className={`text-xs uppercase tracking-[0.22em] ${isDark ? 'text-sky-300' : 'text-sky-700'}`}>Bonuan / Report</p>
          <h1 className={`text-3xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{report.title}</h1>
        </div>
        <div className="flex gap-3">
          <Link to="/barangay/bonuan/reports" className={`inline-flex items-center rounded-xl border px-4 py-2 text-sm font-medium ${isDark ? 'border-[#2e303a] text-gray-200' : 'border-slate-200 text-slate-700'}`}>
            Back to reports
          </Link>
          <button type="button" onClick={() => navigate('/barangay/bonuan/dashboard')} className="inline-flex items-center rounded-xl bg-[#3b82f6] px-4 py-2 text-sm font-semibold text-white hover:bg-[#2563eb]">
            Dashboard
          </button>
        </div>
      </div>
...
export default BonuanReportDetail;
