...
const report = {
  id: 'HR-2214',
  title: 'Waterlogged access road near Tapuac market',
  status: 'Monitoring',
  priority: 'High',
  barangay: 'Tapuac',
  summary: 'A section of the market road remains waterlogged after significant rainfall, affecting vehicle movement and pedestrian access.',
  details: [
    'Roadside drainage channels are being inspected for blockage.',
    'Temporary barriers were placed to alert commuters.',
    'Barangay enforcement team is coordinating with public works for cleanup.'
  ],
};

const TapuacReportDetail = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <p className={`text-xs uppercase tracking-[0.22em] ${isDark ? 'text-sky-300' : 'text-sky-700'}`}>Tapuac / Report</p>
          <h1 className={`text-3xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{report.title}</h1>
        </div>
        <div className="flex gap-3">
          <Link to="/barangay/tapuac/reports" className={`inline-flex items-center rounded-xl border px-4 py-2 text-sm font-medium ${isDark ? 'border-[#2e303a] text-gray-200' : 'border-slate-200 text-slate-700'}`}>
            Back to reports
          </Link>
          <button type="button" onClick={() => navigate('/barangay/tapuac/dashboard')} className="inline-flex items-center rounded-xl bg-[#3b82f6] px-4 py-2 text-sm font-semibold text-white hover:bg-[#2563eb]">
            Dashboard
          </button>
        </div>
      </div>
...
export default TapuacReportDetail;
