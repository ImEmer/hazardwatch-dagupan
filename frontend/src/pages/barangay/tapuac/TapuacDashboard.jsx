...
const stats = [
  { label: 'Open incidents', value: '10', tone: 'text-amber-400' },
  { label: 'Resolved', value: '26', tone: 'text-emerald-400' },
  { label: 'Critical alerts', value: '4', tone: 'text-rose-400' },
  { label: 'This week', value: '8', tone: 'text-sky-400' },
];

const recentReports = [
  { id: 'HR-2214', title: 'Waterlogged access road near Tapuac market', status: 'Monitoring', priority: 'High' },
  { id: 'HR-2221', title: 'Fallen signage near intersection', status: 'Assigned', priority: 'Medium' },
  { id: 'HR-2230', title: 'Trash overflow in public lane', status: 'Queued', priority: 'Low' },
];

const alerts = [
  'Flood-prone areas are being monitored after late rain.',
  'Public road inspection team requested for blocked intersection.',
  'Utility crew dispatched for electrical hazard report.',
];

const TapuacDashboard = () => {
  const { theme } = useTheme();
  const isDark = theme === 'dark';
  const basePath = '/barangay/tapuac';

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <p className={`text-xs uppercase tracking-[0.24em] ${isDark ? 'text-sky-300' : 'text-sky-700'}`}>Tapuac</p>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
        <div className={`rounded-2xl border p-5 ${isDark ? 'border-[#2e303a] bg-[#14151d]' : 'border-slate-200 bg-white'}`}>
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className={`text-xs ${isDark ? 'text-sky-300' : 'text-sky-700'}`}>Updated 12 min ago</span>
          </div>
...
export default TapuacDashboard;
