  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      ...
      <div className={`rounded-2xl border p-5 shadow-xl xl:col-span-2 ${isDark ? 'border-[#2e303a] bg-[#14151d]' : 'border-slate-200 bg-white'}`}>
        ...
        <ResponsiveContainer width="100%" height={280}><AreaChart key={theme} data={timelineData} margin={{ top: 8, right: 8, left: 0, bottom: 8 }}><defs><linearGradient id="barangayReportsTrend" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#3b82f6" stopOpacity={0.35} /><stop offset="95%" stopColor="#3b82f6" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke={chartGrid} /><XAxis dataKey="date" stroke={chartText} tick={{ fontSize: 11 }} interval={Math.max(1, Math.floor(daysInMonth / 7))} /><YAxis allowDecimals={false} stroke={chartText} /><Tooltip contentStyle={tooltipStyle} /><Area type="monotone" dataKey="reports" name="Reports" stroke="#3b82f6" fill="url(#barangayReportsTrend)" strokeWidth={2} isAnimationActive animationBegin={0} animationDuration={800} animationEasing="ease-out" /><Line type="monotone" dataKey="reports" stroke="#60a5fa" strokeWidth={2} dot={false} isAnimationActive animationBegin={0} animationDuration={800} animationEasing="ease-out" /></AreaChart></ResponsiveContainer>
      </div>
    </div>
