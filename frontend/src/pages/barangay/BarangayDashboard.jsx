...
import { PRIORITY_COLORS, REPORT_STATUSES, STATUS_CHART_COLORS } from '../../services/reportOptions';
import { SkeletonDashboard } from '../../components/common/Skeleton';
import CountUp from '../../components/common/CountUp';

const priorities = ['Urgent', 'High', 'Medium', 'Low'].map((name) => ({ name, color: PRIORITY_COLORS[name].hex }));
...
const BarangayDashboard = () => {
  ...
  return (
    <div className="space-y-6">
      <header>
        <p className={`text-xs uppercase tracking-[0.25em] ${isDark ? 'text-sky-300' : 'text-sky-700'}`}>{barangay}</p>
        <h1 className={`mt-2 text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          ['Total reports', dashboard.total, 'border-blue-500'],
          ['Pending', statusCounts.Pending || 0, 'border-yellow-500'],
          ['In progress', statusCounts['In Progress'] || 0, 'border-blue-500'],
          ['Resolved', statusCounts.Resolved || 0, 'border-emerald-500'],
        ].map(([label, value, tone]) => (
          <div
            key={label}
            className={`rounded-2xl border-l-4 ${tone} border-y border-r p-4 shadow-xl ${
              isDark ? 'border-y-[#2e303a] border-r-[#2e303a] bg-[#14151d]' : 'border-y-slate-200 border-r-slate-200 bg-white'
            }`}
          >
            <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>{label}</p>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <div className={`rounded-2xl border p-5 shadow-xl ${isDark ? 'border-[#2e303a] bg-[#14151d]' : 'border-slate-200 bg-white'}`}>
          <h2 className={`mb-4 text-xl font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>Reports by status</h2>
            ...
            {priorityData.map((item) => (
              <div key={item.name}>
                <div className={`mb-1.5 flex items-center justify-between text-sm ${isDark ? 'text-gray-300' : 'text-slate-600'}`}>
                  ...
                  <strong className={isDark ? 'text-white' : 'text-slate-900'}>{item.count}</strong>
                </div>
                <div className={`h-2 overflow-hidden rounded-full ${isDark ? 'bg-[#0a0b0f]' : 'bg-slate-100'}`}>
                  <div
                    className="h-full rounded-full transition-all duration-1000 ease-out"
                    style={{
                      width: priorityAnimated ? `${Math.max((item.count / maxPriority) * 100, item.count ? 8 : 0)}%` : '0%',
        ...
        <div className={`rounded-2xl border p-5 shadow-xl xl:col-span-2 ${isDark ? 'border-[#2e303a] bg-[#14151d]' : 'border-slate-200 bg-white'}`}>
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>Reports over time</h2>
              <p className={`mt-1 text-xs ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>
                Daily submissions for the selected month
              </p>
            </div>
              ...
              {monthOptions.map((option) => (
                <button
                  ...
                  className={`rounded-lg px-2.5 py-1 text-xs font-medium transition ${
                    month === option.month ? 'bg-[#3b82f6] text-white' : isDark ? 'bg-[#0a0b0f] text-gray-400 hover:text-white' : 'bg-slate-100 text-slate-600 hover:text-slate-900'
                  }`}
                >
      ...
      <section className={`rounded-2xl border p-5 shadow-xl ${isDark ? 'border-[#2e303a] bg-[#14151d]' : 'border-slate-200 bg-white'}`}>
        <div className="flex items-center justify-between gap-3">
          <h2 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>Recent reports</h2>
          <button type="button" onClick={() => navigate('/barangay/reports')} className="text-sm font-semibold text-[#3b82f6] hover:text-[#60a5fa]">
            View All
          </button>
        </div>
        ...
        <div className="mt-4 space-y-3">
          {dashboard.reports.map((report) => (
            <div
              key={report._id}
              className={`flex items-center justify-between rounded-xl border p-3 ${isDark ? 'border-[#2e303a] bg-[#0a0b0f]' : 'border-slate-200 bg-slate-50'}`}
            >
              <div>
                <p className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                  {report.title || `${report.category || 'Hazard'} report`}
                </p>
                <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>{report._id}</p>
              </div>
              <div className="text-right">
                <p className="text-xs font-medium text-[#60a5fa]">{report.priority || 'Medium'}</p>
                <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>{report.status || 'Pending'}</p>
              </div>
            </div>
          ))}

          {!dashboard.reports.length && (
            <p className={`py-6 text-center text-sm ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>
              No reports found.
            </p>
          )}
        </div>
