...
const LoginPage = () => {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (loginError.data?.needsVerification) {
        const email = loginError.data.email || form.email.trim().toLowerCase();
        await showError('Email verification required', loginError.message);
